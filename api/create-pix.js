...
const PRODUCT_NAME = 'Combo Base 360°';
const PRODUCT_AMOUNT = 39700;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payload = {
      ...
      externalRef: `combo-base-360-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    ...
    if (!copypaste) {
      return res.status(502).json({ error: 'A PayShark não retornou o código Pix.' });
    }
